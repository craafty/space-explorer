...
{
    // Convert double precision position to float for the view matrix
    glm::vec3 floatPos = glm::vec3(Position);
    return glm::lookAt(floatPos, floatPos + Front, Up);
}
...
void Camera::ProcessKeyboard(Camera_Movement direction, float deltaTime)
{
    ...
    if (direction == FORWARD)
        Position += glm::dvec3(Front) * (double)velocity;
    if (direction == BACKWARD)
        Position -= glm::dvec3(Front) * (double)velocity;
    if (direction == LEFT)
        Position -= glm::dvec3(Right) * (double)velocity;
    if (direction == RIGHT)
        Position += glm::dvec3(Right) * (double)velocity;
    if (direction == UP)
        Position += glm::dvec3(Up) * (double)velocity;
    if (direction == DOWN)
        Position -= glm::dvec3(Up) * (double)velocity;
}
